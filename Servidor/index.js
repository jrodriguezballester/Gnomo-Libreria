...
app.post('/api/libros', (req, res) => {
    let data = { name: req.body.name, insb: req.body.insb, id_autor: req.body.id_autor };
...
app.patch('/api/libros/:id', (req, res) => {
    let sql = "UPDATE book SET name='" + req.body.name + "', insb='" + req.body.insb + "', id_autor='" + req.body.id_autor + "' WHERE id=" + req.params.id;
    let query = conn.query(sql, (err, results) => {
        if (err) throw err;
        res.send(JSON.stringify({ "status": 200, "error": null, "response": results }));
    });
...
app.patch('/api/autor/:id', (req, res) => {
    let sql = "UPDATE author SET first_name='" + req.body.first_name + "', last_name='" + req.body.last_name + "' WHERE id=" + req.params.id;
    let query = conn.query(sql, (err, results) => {
        if (err) throw err;
        res.send(JSON.stringify({ "status": 200, "error": null, "response": results }));
    });
